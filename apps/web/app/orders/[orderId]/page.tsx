...
import { Package, Clock, CheckCircle, XCircle, ArrowLeft, RefreshCw, Loader2, AlertTriangle } from 'lucide-react';
import { expressoApi, Order, OrderStatus, ManageOrderInput, ExpressoApiError } from '@/lib/api/expresso-api';
...
const statusConfig: Record<OrderStatus, { icon: typeof Package; color: string; bg: string; label: string }> = {
  pending: { icon: Clock, color: 'var(--warning)', bg: 'rgba(245, 158, 11, 0.1)', label: 'Pending' },
  preparing: { icon: Package, color: 'var(--info)', bg: 'rgba(59, 130, 246, 0.1)', label: 'Preparing' },
  prepared: { icon: CheckCircle, color: 'var(--success)', bg: 'rgba(34, 197, 94, 0.1)', label: 'Prepared' },
  cancelled: { icon: XCircle, color: 'var(--destructive)', bg: 'rgba(239, 68, 68, 0.1)', label: 'Cancelled' },
};
        ...
        >
          ...
          <dl className="grid sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <dd className="font-medium mt-0.5" style={{ color: 'var(--foreground)' }}>{order.customerName}</dd>
            </div>
            <div>
              <dt style={{ color: 'var(--muted-foreground)' }}>Placed At</dt>
              <dd className="font-medium mt-0.5" style={{ color: 'var(--foreground)' }}>
            ...
            <div>
              ...
              <dd className="font-bold mt-0.5" style={{ color: 'var(--foreground)' }}>
...
function OrderManagePanel({ order, onUpdate }: { order: Order; onUpdate: () => void }) {
  ...
  async function handleAction(input: ManageOrderInput) {
    ...
    try {
      ...
      console.error('[v0] Order action failed:', err);
    } finally {
      setIsLoading(false);
    }
  }
