...
export async function expectNoHorizontalOverflow(page: Page): Promise<void> {
  ...
  expect(size.scrollWidth, 'page must not horizontally overflow the viewport').toBeLessThanOrEqual(
    size.clientWidth + 1
  );
}
...
export async function expectVisualActionable(
  ...
  expect(box.width, `target must be at least ${minWidth}px wide`).toBeGreaterThanOrEqual(
    minWidth
  );
  expect(box.height, `target must be at least ${minHeight}px tall`).toBeGreaterThanOrEqual(
    minHeight
  );
}
...
export async function expectInViewport(locator: Locator): Promise<void> {
  ...
  expect(box.x, 'target left edge must be inside viewport').toBeGreaterThanOrEqual(0);
  expect(box.y, 'target top edge must be inside viewport').toBeGreaterThanOrEqual(0);
  expect(box.x + box.width, 'target right edge must be inside viewport').toBeLessThanOrEqual(
    viewport.width + VIEWPORT_EDGE_TOLERANCE
  );
  expect(
    box.y + box.height,
...
export async function expectCssUtilityContract(page: Page): Promise<void> {
  const expectations: CssUtilityExpectation[] = [
    ...
    { className: 'min-h-screen', property: 'minHeight', value: `${page.viewportSize()?.height}px` },
  ...
  const results = await page.evaluate((items) => {
    return items.map(({ className, property, value }) => {
      ...
      const actual = getComputedStyle(element)[property as keyof CSSStyleDeclaration];
      element.remove();
